...
from slither_core.analyses.data_dependency.data_dependency import compute_dependency
...
from slither_core.core.declarations.enum_top_level import EnumTopLevel
from slither_core.core.declarations.function_top_level import FunctionTopLevel
from slither_core.core.declarations.import_directive import Import
from slither_core.core.declarations.pragma_directive import Pragma
from slither_core.core.declarations.structure_top_level import StructureTopLevel
from slither_core.core.declarations.using_for_top_level import UsingForTopLevel
from slither_core.core.scope.scope import FileScope
from slither_core.core.solidity_types import ElementaryType, TypeAliasTopLevel
...
from slither_core.solc_parsing.declarations.caller_context import CallerContextExpression
from slither_core.solc_parsing.declarations.contract import ContractSolc
from slither_core.solc_parsing.declarations.custom_error import CustomErrorSolc
from slither_core.solc_parsing.declarations.function import FunctionSolc
from slither_core.solc_parsing.declarations.structure_top_level import StructureTopLevelSolc
from slither_core.solc_parsing.declarations.using_for_top_level import UsingForTopLevelSolc
from slither_core.solc_parsing.exceptions import VariableNotFound
from slither_core.solc_parsing.variables.top_level_variable import TopLevelVariableSolc
...
class SlitherCompilationUnitSolc(CallerContextExpression):
    ...
    def parse_top_level_from_json(self, json_data: str) -> bool:
        try:
            ...
            if "ast" in data_loaded:
                self.parse_top_level_from_loaded_json(
        ...
        for top_level_data in data_loaded[self.get_children()]:
            if top_level_data[self.get_key()] == "ContractDefinition":
                ...
                if "src" in top_level_data:
                    contract.set_offset(
                        top_level_data["src"], self._compilation_unit)
                ...
                if self._is_compact_ast:
                    ...
                    pragma = Pragma(
                        top_level_data["attributes"]["literals"], scope)
                    scope.pragmas.add(pragma)
                pragma.set_offset(
                    top_level_data["src"], self._compilation_unit)
                ...
                usingFor_parser = UsingForTopLevelSolc(
                    usingFor, top_level_data, self)
                usingFor.set_offset(
                    top_level_data["src"], self._compilation_unit)
                ...
                if self.is_compact_ast:
                    ...
                    if "symbolAliases" in top_level_data:
                        ...
                        _handle_import_aliases(
                            symbol_aliases, import_directive, scope)
                else:
                    import_directive = Import(
                        Path(
                ...
                custom_error = CustomErrorTopLevel(
                    self._compilation_unit, scope)
                custom_error.set_offset(
                    top_level_data["src"], self._compilation_unit)

                custom_error_parser = CustomErrorSolc(
                    custom_error, top_level_data, self)
                ...
                user_defined_type = TypeAliasTopLevel(
                    original_type, alias, scope)
                user_defined_type.set_offset(
                    top_level_data["src"], self._compilation_unit)
    ...
    def _parse_source_unit(self, data: Dict, filename: str) -> None:
        ...
        if "src" in data:
            sourceUnit_candidates = re.findall(
                "[0-9]*:[0-9]*:([0-9]*)", data["src"])
        ...
        print(self._compilation_unit.core.source_code)
...
Please rename it, this name is reserved for Slither's internals"""
    ...
    def _analyze_top_level_structures(self) -> None:
        try:
            ...
            raise SlitherException(
                f"Missing struct {e} during top level structure analyze") from e
    ...
    def _analyze_params_top_level_function(self) -> None:
        for func_parser in self._functions_top_level_parser:
            ...
            self._compilation_unit.add_function(
                func_parser.underlying_function)
